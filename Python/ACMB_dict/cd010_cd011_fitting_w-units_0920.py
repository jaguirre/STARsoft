...
xfitopt,xfitcov = curve_fit(fitkids.x_dark_fit,data,xLTD,p0=(.73,1.39,(xtest[0]/f1.to(u.Hz)).value),sigma=xsig,bounds=([0,.5,(min(xLTD)/f1).value],[1,2.5,0]))
...
bounds = ([0,.5,(min(xLTD)/f1.to(u.Hz)).value,0],[1,2.5,(-min(xLTD)/f1.to(u.Hz)).value,max(QinvLTD)])
