...
xtest = kids.xMB(alpha,f,TstageLTD,Tc,TBB,V,n_star,tau_max,eta_pb,eta_opt=1,trans=0)#,N0=N0)
Qinvtest = kids.QinvMB(alpha,f,TstageLTD,Tc,TBB,V,n_star,tau_max,eta_pb,eta_opt=1,trans=0)#,N0=N0)
data = [TstageLTD,f]
xsig = 0.05*xLTD[1]*np.ones_like(xLTD)
Qsig = 0.05*QinvLTD

xfitopt,xfitcov = curve_fit(fitkids.x_dark_fit,data,xLTD,p0=(.73,1.39,(xtest[0]/f)),sigma=xsig,bounds=([0,.5,(min(xLTD)/f)],[1,2.5,0]))#p0=(.73,1.39,(xtest[0]/f).value),sigma=xsig,bounds=([0,.5,(min(xLTD)/f).value],[1,2.5,0]))
...
p0=(.73,1.39,(xtest[0]/f),Qinvtest[0])
#p0=(.73,1.39,(xtest[0]/f).value,Qinvtest[0].value)
cfitopt,cfitcov = curve_fit(fitkids.x_Qinv_dark_simulfit,data,cdata,sigma=csigma,p0=p0,bounds=([0,.5,(min(xLTD)/f),0],[1,2.5,(-min(xLTD)/f),max(QinvLTD)]))#bounds=([0,.5,(min(xLTD)/f).value,0],[1,2.5,(-min(xLTD)/f).value,max(QinvLTD)]))
...
x2test = kids.xMB(alpha,f,Tstage0,Tc,T_BB,V,n_star,tau_max,eta_pb,trans=1,eta_opt=.17)
sxx2test = kids.Sxx(alpha,f,Tstage0,Tc,T_BB,V,n_star,tau_max,eta_pb,nu_opt,eta_opt=0.17,trans=1)#*1e17

data2 = [T_BB,alpha,f,Tstage0,Tc,V,eta_pb,nu_opt,trans,N0_Al]
p0x = (n_star,tau_max,eta_opt,(x2test[1]/f))
boundsx = ([0,0,0,(min(x2_interp)/f)],[np.inf,1e5,1,-(min(x2_interp)/f)])
x2sig = 0.05*x2_interp[1]*np.ones_like(x2_interp)

p0Sxx = (n_star,tau_max,eta_opt,(Sxx_interp[0]/2))
boundsSxx = ([0,0,0,0],[np.inf,1e5,1,(Sxx_interp[0])])
...
opt_p0 =(n_star,tau_max,eta_opt,(x2test[1]/f),(Sxx_interp[0]/2))
opt_bounds = ([1,0,0,(min(x2_interp)/f),0],[np.inf,1e5,1,-(min(x2_interp)/f),(Sxx_interp[0])])
